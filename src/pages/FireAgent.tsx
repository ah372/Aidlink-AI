...
import { fireChat, fireVoiceChat, getFireChatHistory, generateUserId, getAudioUrl } from '@/lib/api';
...
const FireAgent = () => {
  ...
  const handleVoiceMessage = async (audioBlob: Blob) => {
    ...
    try {
      const response = await fireVoiceChat(userId, audioBlob);
      ...
      console.error('Failed to send voice message to fire agent:', error);
      toast({
  ...
  return (
    <div className="h-screen flex flex-col">
      {/* Navigation Bar */}
      <div 
        className="flex justify-between items-center p-4 bg-gradient-fire border-b border-fire-border relative bg-cover bg-center"
        style={{ backgroundImage: `url(${fireHeroImage})` }}
      >
        <div className="absolute inset-0 bg-fire-primary/80"></div>
